...
class ArticleAnalyzer(HtmlAnalyzer):
    ...
    def create_article(self, beautiful_soup: BeautifulSoup, tag_article_area: Tag) -> Article:
        list_url_image: List[str] = list({
            image_tag.get('src') for image_tag in tag_article_area.find_all('img')
    ...
    def is_target_image_tag(cls, image_tag: Tag) -> bool:
        return (
            image_tag.has_attr('src') and
            image_tag.get('src').find('//d-markets.net') == -1 and
            (not image_tag.has_attr('width') or int(image_tag.get('width')) > cls.PIXEL_EMOJI_AMEBA_BLOG) and
            (not image_tag.has_attr('height') or int(image_tag.get('height')) > cls.PIXEL_EMOJI_AMEBA_BLOG) and
            (not image_tag.get('src').startswith('cid'))
        )
